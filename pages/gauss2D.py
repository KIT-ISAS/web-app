...
fig = go.Figure()
fig.add_trace(go.Scattergl(name='Density', x=[0], y=[0], mode='lines',   marker_color=col_density, showlegend=True, hoverinfo='skip', line={'width': 3}, line_shape='linear', fill='tozerox'))
fig.add_trace(go.Scattergl(name='Samples', x=[0], y=[0], mode='markers', marker_color=col_samples, marker_line_color='black', marker_opacity=1, showlegend=True))
...
layout =SplitPane([
	dbc.Container(
		dbc.Col([
			...
			# Info Popup
			*PopupBox("gauss2D-info", "Learn More", "Additional Information", info_text),
		]), 
		fluid=True,
		className="g-0")
	],
	[
		# Plot
		dcc.Graph(id="gauss2D-graph", figure=fig, config=config, style={'height': '100%'}),
	],
	30
)
